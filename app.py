...
# Save or update FAISS vector store from text chunks
def update_vector_store(topic, text_chunks):
    embeddings = GoogleGenerativeAIEmbeddings(model="models/embedding-001")
    topic_index_path = os.path.join(INDEX_DIR, f"{topic}_faiss_index")
    
    # Load existing vector store if available
    if os.path.exists(topic_index_path):
        vector_store = FAISS.load_local(topic_index_path, embeddings, allow_dangerous_deserialization=True)
            ...
            if pdf_docs and topic.strip():
                with st.spinner("Processing..."):
                    ...
                    if raw_text.strip():
                        ...
                        update_vector_store(topic.strip(), text_chunks)
                        st.success(f"Processing complete for topic: {topic.strip()}! Data has been updated.")
                    else:
